...
export function getPhrasesForUnit(unitId: number): UnitPhrases {
    ...
    if (unitId >= 41 && unitId <= 60) {
        const b1Phrases = getB1PhrasesForUnit(unitId);
        return {
            unitId,
            title: `Ünite ${unitId}`,
            phrases: b1Phrases.map(p => ({
                target: p.target,
                native: p.native,
                pronunciation: p.pronunciation
            }))
    ...
    if (unitId >= 26 && unitId <= 40) {
        return getA2PhrasesForUnit(unitId);
    }
    ...
    return phrases[unitId] || { unitId, title: "Kalıplar", phrases: [] };
}
